...
# ════════════════════════════════════════════════════════════════
#  PGO / LLVM BOLT  —  REMOVED
#  Toàn bộ logic PGO (profile-guided optimization) và LLVM BOLT đã bị xoá.
#  Luồng build (apt / source build) chạy PGO/BOLT không tồn tại ở chế độ
#  rootless AppImage, nên chỉ giữ lại các global + stub no-op để tương thích
#  với parser và các khối điều kiện (vốn luôn false sau khi tắt cờ).
PGO_MODE=0; PGO_PHASE=""; PGO_PROFILE_READY=0; PGO_PROFILE_KEY=""
PGO_PROFILE_DIR=""; PGO_PROFILE_ROOT=""; PGO_PROFILE_ARCHIVE=""
PGO_PROFILE_KIND="gcc"; PGO_LAUNCH_ENV=""
BOLT_MODE=0; BOLT_PROFILE_KEY=""; BOLT_PROFILE_DIR=""; BOLT_FDATA=""
BOLT_COMPLETE_MARKER=""; BOLT_LLVM_VERSIONS=()
export PGO_MODE PGO_PHASE PGO_PROFILE_READY PGO_PROFILE_KEY PGO_PROFILE_DIR \
       PGO_PROFILE_ROOT PGO_PROFILE_ARCHIVE PGO_PROFILE_KIND PGO_LAUNCH_ENV \
       BOLT_MODE BOLT_PROFILE_KEY BOLT_PROFILE_DIR BOLT_FDATA \
       BOLT_COMPLETE_MARKER BOLT_LLVM_VERSIONS

_pgo_key_for_choice()         { :; }
_bolt_key_for_choice()        { :; }
_pgo_remote_url()             { :; }
_pgo_download_remote()        { return 1; }
_pgo_prepare_context()        { :; }
_bolt_prepare_context()       { :; }
_pgo_stop_vm()                { :; }
_bolt_finalize_after_vm()     { return 0; }
_pgo_finalize_profile()       { :; }
_bolt_check_tools()           { return 1; }
_bolt_find_tool()             { :; }
_bolt_is_ready()              { return 1; }
_bolt_prepare_instrumented()  { return 1; }
_bolt_merge_and_apply()       { return 1; }
...
for _arg in "$@"; do
    case "$_arg" in
        ...
        --pgo)         echo -e "${Y}⚠${W}  --pgo đã bị xoá — không còn hỗ trợ PGO (dùng KVM/AppImage)" ;;
        --llvm-bolt|--bolt) echo -e "${Y}⚠${W}  --llvm-bolt/--bolt đã bị xoá — không còn hỗ trợ LLVM BOLT" ;;
        ...
        --help|-h)
            ...
            echo "  --pgo           (đã xoá) — PGO không còn được hỗ trợ"
            echo "  --llvm-bolt/--bolt  (đã xoá) — LLVM BOLT không còn được hỗ trợ"
...
_detect_kvm() {
    ...
    if [[ ! -e /dev/kvm ]]; then
        echo -e "${Y}⚠${W}  /dev/kvm không tồn tại — dùng LLVM JIT"
        KVM_AVAILABLE=0
        KVM_MODE="tcg"
        return
    fi
    ...
    if [[ "$KVM_OWNER" == "root" ]] && [[ "$KVM_GROUP" == "root" || "$KVM_GROUP" == "kvm" ]]; then
        ...
        echo -e "   Phát hiện     : ${R}${KVM_OWNER}:${KVM_GROUP}${W} → fallback LLVM JIT"
        CAN_USE_KVM=0
    fi
    ...
    if [[ $CAN_USE_KVM -eq 1 ]]; then
        ...
        echo -e "${Y}⚠${W}  Không đủ quyền dùng /dev/kvm — dùng LLVM JIT"
        KVM_AVAILABLE=0
        KVM_MODE="tcg"
    fi
...
_rootless_build() {
    ...
    _rootless_download_appimage() {
        ...
        local _ok=0
        local _urls
        # KVM có sẵn → dùng QEMU AppImage mặc định (hardware accel, như bản gốc)
        # KVM không có → dùng WinBoxes-x86_64.AppImage có LLVM JIT (-accel llvm)
        if [[ "${KVM_AVAILABLE:-0}" == "1" ]]; then
            _urls=(
            ...
            _urls=(
                "https://archive.org/download/win-boxes-x-86-64/WinBoxes-x86_64.AppImage"
            )
        fi
...
# ── WinBoxes LLVM mode: LUÔN dùng AppImage — loại bỏ hoàn toàn
#    "build mode" (apt source build) và "root mode" (apt/root).
#    - KVM có sẵn  → tải QEMU AppImage mặc định (github) + -accel kvm (như cũ)
#    - KVM không có → tải WinBoxes-x86_64.AppImage (archive.org) + -accel llvm
ROOTLESS=1
if [[ "${KVM_AVAILABLE:-0}" == "0" ]]; then
    KVM_MODE="llvm"
fi
export ROOTLESS KVM_MODE
...
_iso_mode_run() {
    ...
    if [[ -r /dev/kvm ]]; then
        ...
        echo -e "${Y}⚠${W}  KVM không có — dùng LLVM JIT"
        ...
        [[ "$_tcg_tb_mb" -gt 8192 ]] && _tcg_tb_mb=8192
         _kvm_accel_args=(-accel llvm)
        echo -e "${G}⚡ LLVM TB cache: ${_tcg_tb_mb}MB | multi-thread${W}"
    ...
    if [[ "$_kvm_ok" == "1" ]]; then
        ...
        echo -e "⚡ Accel      : ${G}LLVM${W} | TB: ${_tcg_tb_mb}MB${W}"
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    echo -e "${C}⚡ Acceleration: ${G}LLVM (Fast)${C}${W}"
fi
echo -e "${C}════════════════════════════════════${W}"
...
" 2>/dev/null
        ...
        if [[ "$KVM_AVAILABLE" == "1" ]]; then
            ...
            echo -e "${B}ℹ${W}  QEMU apt-build: --disable-kvm (LLVM mode)"
        fi
...
_tcg_tune() {
    if [[ "${NO_TUNING:-0}" == "1" ]]; then
        echo -e "${Y}⚠${W}  Bỏ qua toàn bộ LLVM tuning"
        LAUNCH_PREFIX=""
        TCG_TB_MB=512
        return
    fi
    echo ""
    echo -e "${C}════════════════════════════════════${W}"
    echo -e "${C}🔧 LLVM PERFORMANCE TUNING${W}"
    echo -e "${C}════════════════════════════════════${W}"
    _tcg_tune_common
    if [[ $EUID -eq 0 ]]; then
        _tcg_tune_root
    ...
    echo -e "${G}🔥 LLVM tuning xong — full LLVM optimizations on${W}"
    echo ""
}
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    # ── Rootless/portable KVM optimizations ──────────────────────────
    # KVM (hardware accel) là đường nhanh nhất. Vẫn tối ưu thêm:
    #   • pin QEMU vào các core được cgroup cho phép (taskset) để tránh
    #     scheduler di chuyển vCPU giữa cores → jitter thấp hơn.
    #   • bật hugepages khi có thể MÀ không cần root (memfd / /dev/hugepages).
    export MALLOC_ARENA_MAX=4 MALLOC_MMAP_THRESHOLD_=131072 MALLOC_TRIM_THRESHOLD_=131072
    export QEMU_AUDIO_DRV=none
    _kvm_prefix=""
    if command -v taskset &>/dev/null; then
        _kvm_cores="${cpu_u:-${cpu_core:-$(nproc)}}"
        [[ "$_kvm_cores" -lt 1 ]] && _kvm_cores=1
        _kvm_range="0-$(( _kvm_cores - 1 ))"
        [[ "$_kvm_cores" -eq 1 ]] && _kvm_range="0"
        _kvm_prefix="taskset -c $_kvm_range"
        echo -e "${G}✔${W} taskset: pin KVM vCPU → [${_kvm_range}]"
    else
        echo -e "${Y}⚠${W}  taskset không có — bỏ qua CPU pinning (KVM)"
    fi
    LAUNCH_PREFIX="$_kvm_prefix"
    ...
    # ── TCG MODE ─────────────────────────────────────────────────
    echo -e "${G}⚡ VM sẽ chạy với ${G}LLVM (Fast)${W}"
    ...
    echo -e "${G}⚡ LLVM accel: multi-thread + split-wx=off + one-insn-per-tb=off${W}"
    ...
    # Hugepages: ưu tiên QEMU_HUGEPAGES_DIR đã set (root mode), nếu chưa có
    # thì thử /dev/hugepages (rootless — không ghi được sysfs nr_hugepages).
    if [[ -z "${QEMU_HUGEPAGES_DIR:-}" && "$EUID" -ne 0 && -d /dev/hugepages && -w /dev/hugepages ]]; then
        QEMU_HUGEPAGES_DIR="/dev/hugepages"
        export QEMU_HUGEPAGES_DIR
        echo -e "${G}✔${W} Hugepages (rootless): /dev/hugepages"
    fi
...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    echo -e "⚡ Acceleration : ${G}LLVM (Fast)${W}"

...
if [[ "${win_choice:-}" == "3" && "${PGO_MODE:-0}" == "0" && "${KVM_AVAILABLE:-0}" == "0" ]]; then
    ...
    if [[ "$_auto_pgo_ok" == "1" ]]; then
        ...
        if tar -xzf "$_AUTO_PGO_ARCHIVE" -C "$_AUTO_PGO_ROOT" >/dev/null 2>&1; then
            ...
            # Chỉ force rebuild nếu chưa có QEMU built sẵn
            # Nếu đã có QEMU → _detect_existing_qemu sẽ skip build tự động
            _existing_qemu="${HOME}/qemu-optimized/bin/qemu-system-x86_64"
            if [[ ! -x "$_existing_qemu" ]]; then
                AUTO_BUILD="yes"

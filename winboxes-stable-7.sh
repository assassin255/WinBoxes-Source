...
"
        ...
        if [[ $EUID -eq 0 ]]; then
            ...
            "/opt/qemu-optimized/bin/qemu-system-x86_64" --version &>/dev/null && _opt_exec_ok=1
        elif sudo -n true 2>/dev/null; then
        ...
        if [[ "$_opt_exec_ok" == "0" ]]; then
            # sudo cần password / không có, hoặc /opt bị mount noexec — cài vào $HOME thay thế
            if [[ $EUID -ne 0 ]] && ! sudo -n true 2>/dev/null; then
                echo -e "${Y}⚠${W}  sudo không có hoặc cần password — cài vào ~/qemu-optimized thay thế"
            else
                echo -e "${Y}⚠${W}  /opt/qemu-optimized cài xong nhưng KHÔNG thực thi được (mount noexec?) — cài vào ~/qemu-optimized thay thế"
            fi

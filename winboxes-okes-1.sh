...
BOLT_PROFILE_DIR="/tmp/qemu-bolt-prof"
...
_bolt_is_ready() {
    ...
    [[ -f "$BOLT_COMPLETE_MARKER" ]] && return 0
    return 1
}
...
_bolt_prepare_instrumented() {
    ...
    local _fdata_files=()
    local _fcount=0
    if compgen -G "${BOLT_PROFILE_DIR}/bolt-prof*" >/dev/null 2>&1; then
        while IFS= read -r -d '' _f; do
            local _fsz; _fsz=$(stat -c%s "$_f" 2>/dev/null || echo 0)
            [[ "$_fsz" -gt 100 ]] || continue
            _fdata_files+=("$_f")
            _fcount=$((_fcount + 1))
        done < <(find "$BOLT_PROFILE_DIR" -maxdepth 1 \( -name "bolt-prof*" -o -name "*.fdata" \) -type f -print0 2>/dev/null)
    fi
    ...
    if "$_bolt_bin" -instrument "$BOLT_ORIG_BIN" \
        -o "$BOLT_INST_BIN" \
        -instrumentation-file="${BOLT_PROFILE_DIR}/bolt-prof" \
        ...
        echo -e "${B}ℹ${W}  Profile sẽ tự động ghi mỗi 60s vào: ${BOLT_PROFILE_DIR}/"
...
_bolt_merge_and_apply() {
    ...
    rm -f "$BOLT_FDATA"
    "$_merge_bin" "${_fdata_files[@]}" -o "$BOLT_FDATA" 2>/tmp/bolt-merge.log || {
    ...
    local _fdata_size; _fdata_size=$(stat -c%s "$BOLT_FDATA" 2>/dev/null || echo 0)
    if [[ "$_fdata_size" -lt 100 ]]; then
        echo -e "${R}✘${W}  fdata file quá nhỏ (${_fdata_size} bytes) — không đủ profile để tối ưu"
        return 1
    fi
    echo -e "${G}✔${W}  fdata merged: ${BOLT_FDATA} (${_fdata_size} bytes)"
    ...
    echo -e "${B}ℹ${W}  fdata:        ${BOLT_FDATA}${W}"
    ...
    if "$_bolt_bin" "$_orig_bin" \
        ...
        # Đánh dấu BOLT đã áp dụng
        echo "$(date -Iseconds)" > "$BOLT_COMPLETE_MARKER"
        BOLT_MODE=2
...
_bolt_finalize_after_vm() {
    ...
    if [[ "$_fcount" -eq 0 ]]; then
        echo -e "${Y}⚠${W}  Không tìm thấy fdata files trong ${BOLT_PROFILE_DIR}"
...
"
        ...
        # LLVM BOLT: prepare instrumented binary sau khi build xong (root mode only)
    ...
    _start_parallel_download
    _wait_parallel_download
    # BOLT: nếu đã có binary từ lần build trước, kiểm tra và finalize nếu cần
    _bolt_prepare_instrumented "$QEMU_BIN" || true
fi

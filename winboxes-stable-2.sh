...
if [[ "$KVM_AVAILABLE" == "1" ]]; then
    ...
    grep -q fma    /proc/cpuinfo && CPU_EXTRA="$CPU_EXTRA,+fma"
    grep -q f16c   /proc/cpuinfo && CPU_EXTRA="$CPU_EXTRA,+f16c"
    grep -q bmi1   /proc/cpuinfo && CPU_EXTRA="$CPU_EXTRA,+bmi1"
    grep -q bmi2   /proc/cpuinfo && CPU_EXTRA="$CPU_EXTRA,+bmi2"
    grep -q invpcid /proc/cpuinfo && CPU_EXTRA="$CPU_EXTRA,+invpcid"
    grep -q pcid   /proc/cpuinfo && CPU_EXTRA="$CPU_EXTRA,+pcid"
    grep -q erms   /proc/cpuinfo && CPU_EXTRA="$CPU_EXTRA,+erms"
    ...
    #
    # AVX-512 / AMX / GFNI / VAES / VPCLMULQDQ bị TẮT TƯỜNG MINH dù base là "max":
    # llvmpipe (Mesa software GL renderer chạy trong guest, dùng cho Minecraft/OpenGL)
    # tự JIT-compile shader code bằng các lệnh SIMD "cao cấp" nếu CPUID quảng cáo
    # chúng có sẵn. TCG hiện dịch sai/thiếu một số opcode trong các nhóm này, gây
    # treo cứng (không crash, không log) ngay tại draw-call GL đầu tiên — điển hình
    # là Minecraft đứng ở loading screen Mojang hoặc không load được world/map.
    # Tắt các nhóm này ép llvmpipe hạ cấp codegen xuống AVX2/SSE, tránh treo.
    cpu_model="max,hypervisor=off,tsc=on,pmu=off,l3-cache=on,+cmov,+mmx,+fxsr,+sse2,+cx16,+x2apic,+sep,+pat,+pse,+aes,+popcnt,-tsc-deadline${CPU_EXTRA},-avx512f,-avx512dq,-avx512cd,-avx512bw,-avx512vl,-avx512-vp2intersect,-avx512bitalg,-avx512vbmi,-avx512vbmi2,-avx512vnni,-avx512ifma,-gfni,-vaes,-vpclmulqdq,-amx-tile,-amx-bf16,-amx-int8,-amx-fp16,model-id=${cpu_model_id}"

...
# ════════════════════════════════════════════════════════════════
#  WINBOX
#  Rootless: dùng QEMU 11 AppImage prebuilt tối ưu (build-time -O3/LTO/native)
#  AppImage tự chứa qemu-system-x86_64, qemu-img, firmware, ROM, libraries
#  Chạy hoàn toàn Rootless, không cần sudo/apt/system-wide QEMU
...
# ════════════════════════════════════════════════════════════════
#  QEMU 11 APPIMAGE BUILD PIPELINE (build-time only, root allowed)
# ════════════════════════════════════════════════════════════════
_build_qemu_appimage() {
    local _appimage_target="${1:-$HOME/.local/share/winboxes/QEMU-11.AppImage}"
    local _build_dir="/tmp/qemu-appimage-build"
    local _qemu_src="/tmp/qemu-src"
    local _appdir="/tmp/AppDir"

    echo -e "${C}══════════════════════════════════════════════${W}"
    echo -e "${C}⚡ BUILDING QEMU 11 OPTIMIZED APPIMAGE${W}"
    echo -e "${C}══════════════════════════════════════════════${W}"
    echo -e "${B}ℹ${W} Build-time optimization: -O3 -flto -march=native -mtune=native -Wl,-O3 -fuse-ld=lld (nếu có)"
    echo -e "${B}ℹ${W} Build-time BOLT (optional): chỉ áp dụng BUILD TIME, không yêu cầu runtime"
    echo -e "${B}ℹ${W} Build-time PGO (optional): chỉ áp dụng BUILD TIME, không phải runtime dependency"

    # Verify source exists or clone
    if [[ ! -d "$_qemu_src" ]]; then
        echo -e "${B}ℹ${W} Cloning QEMU 11 source to $_qemu_src..."
        git clone --depth 1 --branch v11.0.0 https://gitlab.com/qemu-project/qemu.git "$_qemu_src" || {
            echo -e "${R}✘${W} Clone QEMU source thất bại"; return 1;
        }
    fi
    ...
    # Configure với tối ưu mạnh, LTO thực sự hoạt động
    local _lto_flag="-flto"
    local _ldflags="-Wl,-O3 -Wl,--as-needed"
    local _cflags="-O3 -march=native -mtune=native -pipe -fno-plt -fno-semantic-interposition -fomit-frame-pointer -fstack-protector-strong -ffunction-sections -fdata-sections -fipa-cp-clone -fgcse-after-reload -fweb -falign-functions=32 -falign-loops=32 -falign-jumps=32 -falign-labels=32 -fmerge-all-constants -fipa-pta"
    local _fast_math=""
    if [[ "${NO_FAST_MATH:-0}" != "1" ]]; then
        # Áp dụng -ffast-math cho TCG nhưng không cho các thành phần yêu cầu IEEE 754 nghiêm ngặt
        # QEMU TCG FPU không yêu cầu IEEE tuyệt đối, nên có thể dùng
        _cflags="$_cflags -ffast-math"
        _fast_math=" (fast-math enabled, controlled)"
    fi
    _cflags="$_cflags $_lto_flag"
    _ldflags="$_ldflags $_lto_flag"

    # Kiểm tra ld.lld có sẵn không để dùng linker tối ưu
    if command -v ld.lld &>/dev/null; then
        _ldflags="$_ldflags -fuse-ld=lld"
        echo -e "${G}✔${W} Linker: LLVM lld available (tối ưu link time)"
    fi

    echo -e "${B}ℹ${W} CFLAGS: $_cflags${_fast_math}"
    echo -e "${B}ℹ${W} LDFLAGS: $_ldflags"

    # Configure QEMU 11 với TCG (bắt buộc), KVM optional, VNC, firmware
    export CFLAGS="$_cflags"
    export CXXFLAGS="$_cflags"
    export LDFLAGS="$_ldflags"
    export EXTRA_CFLAGS="$_cflags"
    export EXTRA_CXXFLAGS="$_cflags"
    export EXTRA_LDFLAGS="$_ldflags"

    local _kvm_flag="--disable-kvm"
    if [[ -c /dev/kvm ]] && [[ -r /dev/kvm ]]; then
        _kvm_flag="--enable-kvm"
        echo -e "${B}ℹ${W} /dev/kvm available — enabling KVM (TCG vẫn hoạt động không cần KVM)"
    else
        echo -e "${B}ℹ${W} /dev/kvm không khả dụng hoặc không có quyền — dùng TCG (TCG hoàn toàn rootless, không cần KVM)"
    fi

    if "$_qemu_src/configure" \
        --prefix=/usr \
        ...
        --extra-cxxflags="$_cflags" \
        --extra-ldflags="$_ldflags" \
        --enable-lto \
        2>/dev/null; then
        echo -e "${G}✔${W} Configure QEMU 11 xong"
    else
        echo -e "${R}✘${W} Configure QEMU 11 thất bại"; return 1
    fi

    # Build
    echo -e "${B}ℹ${W} Building QEMU 11 with LTO + -O3 + native optimization (mất 10-30 phút)..."
    make -j"$(nproc)" || {
        echo -e "${R}✘${W} QEMU build thất bại"; return 1;
    }

    # Tạo AppDir và đóng gói
    echo -e "${B}ℹ${W} Đóng gói AppImage..."
    rm -rf "$_appdir"
    mkdir -p "$_appdir/usr/bin" "$_appdir/usr/lib" "$_appdir/usr/lib64" "$_appdir/usr/share/qemu" "$_appdir/usr/share/qemu/firmware"

    # Copy binaries
    cp -L build/qemu-system-x86_64 "$_appdir/usr/bin/"
    cp -L build/qemu-img "$_appdir/usr/bin/"
    chmod +x "$_appdir/usr/bin/"*

    # Copy runtime libraries (self-contained)
    # Dùng ldd để tìm dependencies
    for _bin in "$_appdir/usr/bin/"*; do
        if [[ -x "$_bin" ]]; then
            ldd "$_bin" 2>/dev/null | awk '{print $3}' | grep '^/' | sort -u | while read -r _lib; do
                if [[ -f "$_lib" ]]; then
                    cp -L "$_lib" "$_appdir/usr/lib/" 2>/dev/null || cp -L "$_lib" "$_appdir/usr/lib64/" 2>/dev/null || true
                fi
            done
            # Copy thêm lib cần thiết cho TCG và firmware
            for _extra_lib in /usr/lib/x86_64-linux-gnu/libslirp.so* /usr/lib/x86_64-linux-gnu/libglib-2.0.so* /usr/lib/x86_64-linux-gnu/libpixman-1.so* /usr/lib/libstdc++.so* /usr/lib/libgcc_s.so*; do
                if [[ -f "$_extra_lib" ]]; then
                    cp -L "$_extra_lib" "$_appdir/usr/lib/" 2>/dev/null || true
                    cp -L "$_extra_lib" "$_appdir/usr/lib64/" 2>/dev/null || true
                fi
            done
        fi
    done

    # Copy firmware/BIOS/UEFI/ROM/share data
    for _fw_dir in /usr/share/qemu /usr/share/seabios /usr/share/ovmf /usr/share/qemu-efi; do
        if [[ -d "$_fw_dir" ]]; then
            cp -rL "$_fw_dir"/* "$_appdir/usr/share/qemu/" 2>/dev/null || true
        fi
    done
    # Keymaps, modules
    for _share_dir in /usr/share/qemu/keymaps /usr/share/qemu/modules /usr/share/qemu/roms; do
        if [[ -d "$_share_dir" ]]; then
            cp -rL "$_share_dir"/* "$_appdir/usr/share/qemu/" 2>/dev/null || true
        fi
    done

    # Tạo AppRun
    cat > "$_appdir/AppRun" << 'APPIMAGE_EOF'
#!/bin/sh
HERE="$(dirname "$(readlink -f "${0}")")"
export PATH="${HERE}/usr/bin:${PATH}"
export LD_LIBRARY_PATH="${HERE}/usr/lib:${HERE}/usr/lib64:${LD_LIBRARY_PATH:-}"
export QEMU_MODULE_DIR="${HERE}/usr/share/qemu"
export QEMU_FIRMWARE_DIR="${HERE}/usr/share/qemu"
# Nếu AppImage được mount ở vị trí khác
exec "${HERE}/usr/bin/qemu-system-x86_64" "$@"
APPIMAGE_EOF
    chmod +x "$_appdir/AppRun"

    # Tạo AppImage bằng appimagetool (nếu có)
    if command -v appimagetool &>/dev/null; then
        appimagetool --no-appstream "$_appdir" "$_appimage_target"
        echo -e "${G}✔${W} AppImage tạo thành công: ${_appimage_target}"
    else
        echo -e "${Y}⚠${W} appimagetool không có — tạo symlink / script wrapper thay thế AppImage đóng gói"
        # Tạo executable script wrapper tự chứa (thay thế AppImage thực sự)
        cat > "$_appimage_target" << 'WRAPPER_EOF'
#!/usr/bin/env bash
# Self-contained QEMU 11 wrapper (AppImage-equivalent)
HERE="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" && pwd)"
# Nếu file này được symlink đến AppDir, tìm thư mục thực
if [[ -L "${BASH_SOURCE[0]:-$0}" ]]; then
    REAL_FILE=$(readlink -f "${BASH_SOURCE[0]:-$0}")
    HERE=$(dirname "$REAL_FILE")
fi
export PATH="${HERE}/usr/bin:${PATH}"
export LD_LIBRARY_PATH="${HERE}/usr/lib:${HERE}/usr/lib64:${LD_LIBRARY_PATH:-}"
export QEMU_MODULE_DIR="${HERE}/usr/share/qemu"
if [[ "$1" == "qemu-system-x86_64" ]] || [[ "$1" == "--version" ]] || [[ -z "$1" ]]; then
    exec "${HERE}/usr/bin/qemu-system-x86_64" "$@"
fi
exec "${HERE}/usr/bin/qemu-system-x86_64" "$@"
WRAPPER_EOF
        chmod +x "$_appimage_target"
        echo -e "${G}✔${W} Self-contained wrapper tạo tại: $_appimage_target"
    fi

    echo -e "${B}ℹ${W} QEMU 11 AppImage build hoàn tất. Validate bằng: $_appimage_target --version"
    return 0
}
...
if [[ "$DELETE_BUILD_MODE" == "1" ]]; then
    ...
    _del_dir "/tmp/qemu-appimage-build"    "QEMU AppImage build dir"
...
_rootless_build() {
    ...
    _rootless_download_appimage() {
        ...
        local _ok=0
        local _urls=(
            "https://github.com/assassin255/WinBoxes-Source/raw/refs/heads/main/QEMU-11-Optimize.AppImage.tar.xz"
        ...
        for _url in "${_urls[@]}"; do
            ...
            if [[ "$_ok" == "1" ]] && [[ -s "$_dest" ]]; then
                ...
                if [[ "$_dest" == *.tar.xz ]] && command -v tar &>/dev/null; then
...
_detect_existing_qemu() {
    ...
    if [[ -n "$_appimg_path" && -x "$_appimg_path" ]]; then
        ...
        if [[ "$_app_ver" == 11.* ]] || [[ "$_app_ver" == unknown ]]; then
            ...
            echo -e "${G}✔${W} Rootless mode: ${B}enabled${W} (no sudo/apt required)"
            echo -e "${G}✔${W} TCG available: ${B}yes${W} (TCG works without /dev/kvm)"
            echo -e "${G}✔${W} LTO optimized: ${B}baked-in${W} (AppImage build-time LTO)"
            echo -e "${G}✔${W} -O3 optimized: ${B}baked-in${W} (native CPU optimized)"
            return 0
        fi
    fi

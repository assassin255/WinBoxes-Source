...
# ── Tải file với fallback chống lỗi SSL handshake ────────────────
# aria2 static-musl build đôi khi lỗi "SSL/TLS handshake failure: SSL I/O error"
# (TLS backend nhúng trong static binary không tương thích/ổn định với server
# dùng TLS1.3 như archive.org, đặc biệt khi multi-connection cao). Chuỗi fallback:
#   aria2c (16 connection) → aria2c (2 connection, retry SSL) → wget → curl
# Trả về 0 nếu tải xong, 1 nếu tất cả đều thất bại.
_download_with_fallback() {
    local _url="$1" _dest="$2" _log="${3:-/tmp/dl-fallback-$$.log}"
    local _dir _name _rc
    _dir="$(dirname "$_dest")"; _name="$(basename "$_dest")"

    if command -v aria2c &>/dev/null; then
        aria2c "${ARIA2_OPTS[@]}" "$_url" -d "$_dir" -o "$_name" > "$_log" 2>&1
        _rc=$?
        if [[ $_rc -eq 0 && -f "$_dest" ]]; then
            return 0
        fi
        if grep -qi "SSL/TLS handshake\|SSL I/O error" "$_log" 2>/dev/null; then
            echo -e "${Y}⚠${W}  aria2c SSL handshake lỗi (static-musl TLS backend) — thử lại ít connection hơn..." >&2
            aria2c --split=2 --max-connection-per-server=2 --check-certificate=false \
                --continue=true --file-allocation=none --max-tries=3 --retry-wait=3 \
                --timeout=60 --connect-timeout=15 \
                "$_url" -d "$_dir" -o "$_name" >> "$_log" 2>&1
            _rc=$?
            [[ $_rc -eq 0 && -f "$_dest" ]] && return 0
        fi
        echo -e "${Y}⚠${W}  aria2c thất bại (xem $_log) — thử wget..." >&2
    fi

    if command -v wget &>/dev/null; then
        wget --progress=dot:giga --continue --tries=5 --timeout=60 \
            --no-check-certificate "$_url" -O "$_dest" >> "$_log" 2>&1
        _rc=$?
        [[ $_rc -eq 0 && -f "$_dest" ]] && return 0
        echo -e "${Y}⚠${W}  wget thất bại (xem $_log) — thử curl..." >&2
    fi

    if command -v curl &>/dev/null; then
        curl -k -L --retry 5 --retry-delay 3 --connect-timeout 15 \
            -C - -o "$_dest" "$_url" >> "$_log" 2>&1
        _rc=$?
        [[ $_rc -eq 0 && -f "$_dest" ]] && return 0
    fi
...
_start_parallel_download() {
    ...
    (
        _download_with_fallback "$WIN_URL" "${WIN_IMG_PATH:-win.img}" "/tmp/dl-parallel.log"
    ) &
    IMG_DL_PID=$!
...
if [[ "$_HTTP_BACKED" == "1" ]] || [[ "${_IMG_DOWNLOAD_DONE:-0}" == "1" ]] || _img_valid "$WIN_IMG_PATH"; then
    ...
    echo -e "${C}════════════════════════════════════${W}"
    if _download_with_fallback "$WIN_URL" "$WIN_IMG_PATH" "/tmp/dl-win-img-$$.log"; then
        echo -e "${G}✔ Tải $WIN_NAME xong${W}"
    else
        echo -e "${R}✘ Tải $WIN_NAME thất bại (aria2c + wget + curl đều lỗi) — xem /tmp/dl-win-img-$$.log${W}"
        tail -n 30 "/tmp/dl-win-img-$$.log" 2>/dev/null || true
        exit 1
    fi
fi

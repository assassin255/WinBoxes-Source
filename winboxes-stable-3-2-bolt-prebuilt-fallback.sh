...
# ════════════════════════════════════════════════════════════════
#  LLVM/BOLT PREBUILT FALLBACK
#  LLVM không phát hành "AppImage" chính thức. Bản tương đương ổn định
#  nhất là tarball nhị phân chính thức (clang+llvm-*.tar.xz) trên GitHub
#  Releases của llvm/llvm-project: tự chứa (self-contained), không phụ
#  thuộc gói apt của từng distro (vốn hay thiếu/lỗi như trong lỗi gặp
#  phải). Dùng làm fallback khi apt không có llvm-bolt/merge-fdata.
# ════════════════════════════════════════════════════════════════

BOLT_PREBUILT_ROOT="$HOME/.cache/qemu-bolt-llvm"
BOLT_PREBUILT_BIN_DIR="$BOLT_PREBUILT_ROOT/wrappers"

# Bản LLVM "final" đã biết có phát hành tarball x86_64 Linux + kèm BOLT,
# khớp với các version trong BOLT_LLVM_VERSIONS ở trên (mới → cũ).
declare -A BOLT_LLVM_FULLVER=(
    [21]="21.1.3"  [20]="20.1.8"  [19]="19.1.0"  [18]="18.1.8"
    [17]="17.0.6"  [16]="16.0.6"  [15]="15.0.7"  [14]="14.0.6"
    [13]="13.0.1"
)

# _bolt_asset_url_via_api <full_version>: hỏi GitHub API danh sách asset
# của release để lấy đúng tên file tar.xz x86_64 Linux (tên file đổi
# thất thường giữa các version LLVM, xem llvm/llvm-project#68804/#60201).
_bolt_asset_url_via_api() {
    local _fv="$1" _tag="llvmorg-${1}" _json _url
    command -v curl &>/dev/null || return 1
    _json=$(curl -fsSL --max-time 15 \
        "https://api.github.com/repos/llvm/llvm-project/releases/tags/${_tag}" 2>/dev/null) || return 1
    [[ -z "$_json" ]] && return 1
    _url=$(grep -oE '"browser_download_url": *"[^"]*clang\+llvm-[^"]*x86_64[^"]*linux[^"]*\.tar\.(xz|zst)"' <<<"$_json" \
        | grep -viE 'windows|darwin|solaris|aix' \
        | head -1 \
        | sed -E 's/.*"(https:[^"]+)".*/\1/')
...
# _bolt_asset_url_guess <full_version>: đoán URL trực tiếp — fallback khi
# GitHub API không gọi được (rate limit / mất mạng lúc đó).
_bolt_asset_url_guess() {
    local _fv="$1" _tag="llvmorg-${1}" _suf
    for _suf in "linux-gnu-ubuntu-22.04" "linux-gnu-ubuntu-20.04" "linux-gnu-ubuntu-24.04" "linux-gnu" "unknown-linux-gnu"; do
        echo "https://github.com/llvm/llvm-project/releases/download/${_tag}/clang+llvm-${_fv}-x86_64-${_suf}.tar.xz"
    done
}

# _bolt_download_file <url> <dest>: tải 1 file, aria2c → wget → curl,
# dùng chung ARIA2_OPTS (định nghĩa ở phần package manager bên dưới).
_bolt_download_file() {
    local _url="$1" _dest="$2" _dir _name
    _dir="$(dirname "$_dest")"; _name="$(basename "$_dest")"
    mkdir -p "$_dir"
    if command -v aria2c &>/dev/null; then
        aria2c "${ARIA2_OPTS[@]}" --out="$_name" --dir="$_dir" "$_url" \
            >/tmp/bolt-llvm-download.log 2>&1 && return 0
    fi
    if command -v wget &>/dev/null; then
        wget -c --no-check-certificate --progress=bar:force:noscroll -O "$_dest" "$_url" \
            >>/tmp/bolt-llvm-download.log 2>&1 && return 0
    fi
    curl -fL --insecure --retry 5 --retry-delay 3 -o "$_dest" "$_url" \
        >>/tmp/bolt-llvm-download.log 2>&1 && return 0
    return 1
}

# _bolt_mk_wrapper <real_bin> <wrapper_name>: tạo wrapper trong
# BOLT_PREBUILT_BIN_DIR, set LD_LIBRARY_PATH trỏ vào lib/ đi kèm tarball
# (đề phòng binary link động vào libLLVM*.so trong cùng gói).
_bolt_mk_wrapper() {
    local _real="$1" _name="$2" _libdir="$3"
    [[ -x "$_real" ]] || return 1
    mkdir -p "$BOLT_PREBUILT_BIN_DIR"
    cat > "$BOLT_PREBUILT_BIN_DIR/$_name" <<EOF
#!/bin/bash
export LD_LIBRARY_PATH="${_libdir}:\${LD_LIBRARY_PATH:-}"
exec "${_real}" "\$@"
EOF
    chmod +x "$BOLT_PREBUILT_BIN_DIR/$_name"
}

# _bolt_link_prebuilt <version> <install_dir>: tạo wrapper llvm-bolt-N /
# merge-fdata-N và thêm vào PATH để _bolt_find_tool tìm thấy như bình thường.
_bolt_link_prebuilt() {
    local _v="$1" _dir="$2"
    [[ -x "$_dir/bin/llvm-bolt" ]] || return 1

    _bolt_mk_wrapper "$_dir/bin/llvm-bolt" "llvm-bolt-${_v}" "$_dir/lib"
    [[ -x "$_dir/bin/merge-fdata" ]] && _bolt_mk_wrapper "$_dir/bin/merge-fdata" "merge-fdata-${_v}" "$_dir/lib"

    case ":$PATH:" in
        *":$BOLT_PREBUILT_BIN_DIR:"*) ;;
        *) export PATH="$BOLT_PREBUILT_BIN_DIR:$PATH" ;;
    esac

    # libbolt_rt_instr.a (cần cho -instrument) đôi khi có kèm trong lib/ —
    # nếu có, symlink vào /usr/lib để _bolt_ensure_runtime_lib tìm thấy.
    if [[ -f "$_dir/lib/libbolt_rt_instr.a" && ! -f /usr/lib/libbolt_rt_instr.a ]]; then
        ln -sf "$_dir/lib/libbolt_rt_instr.a" /usr/lib/libbolt_rt_instr.a 2>/dev/null \
            || sudo ln -sf "$_dir/lib/libbolt_rt_instr.a" /usr/lib/libbolt_rt_instr.a 2>/dev/null || true
    fi

    command -v "llvm-bolt-${_v}" &>/dev/null
}

# _bolt_download_prebuilt_llvm: tải + giải nén tarball LLVM chính thức,
# thử lần lượt từ bản mới nhất tới cũ nhất trong BOLT_LLVM_FULLVER cho
# tới khi có bin/llvm-bolt hợp lệ. Cache lại theo version trong
# BOLT_PREBUILT_ROOT nên lần chạy sau không cần tải lại.
_bolt_download_prebuilt_llvm() {
    [[ "${NO_BOLT:-0}" == "1" ]] && return 1
    mkdir -p "$BOLT_PREBUILT_ROOT" "$BOLT_PREBUILT_BIN_DIR"

    # Tarball LLVM khá lớn (700MB-1GB+) — đảm bảo có aria2c (đa luồng,
    # resume) trước khi tải để nhanh hơn nhiều so với wget/curl đơn luồng.
    if ! command -v aria2c &>/dev/null; then
        echo -e "${B}ℹ${W}  Cài aria2c để tải LLVM đa luồng cho nhanh..."
        _ensure_aria2 || true
    fi

    local _v _fv
    for _v in "${BOLT_LLVM_VERSIONS[@]}"; do
        _fv="${BOLT_LLVM_FULLVER[$_v]:-}"
        [[ -z "$_fv" ]] && continue

        local _install_dir="$BOLT_PREBUILT_ROOT/$_v"
        # Cache hit — đã tải + giải nén ở lần chạy trước
        if [[ -x "$_install_dir/bin/llvm-bolt" ]]; then
            echo -e "${G}✔${W}  LLVM ${_fv} (BOLT) đã có trong cache — bỏ qua tải"
            _bolt_link_prebuilt "$_v" "$_install_dir" && return 0
            continue
        fi

        echo -e "${B}ℹ${W}  Thử tải LLVM ${_fv} (kèm BOLT) từ GitHub Releases..."
        local _tar="$BOLT_PREBUILT_ROOT/clang+llvm-${_fv}.tar.xz"
        local _candidates=() _api_url
        _api_url="$(_bolt_asset_url_via_api "$_fv" || true)"
        [[ -n "$_api_url" ]] && _candidates+=("$_api_url")
        while IFS= read -r _u; do _candidates+=("$_u"); done < <(_bolt_asset_url_guess "$_fv")

        local _got=0 _u
        for _u in "${_candidates[@]}"; do
            [[ -z "$_u" ]] && continue
            if _bolt_download_file "$_u" "$_tar" && tar tf "$_tar" &>/dev/null; then
                _got=1
                break
            fi
            echo -e "${Y}⚠${W}  Không tải được (hoặc file lỗi): $_u"
            rm -f "$_tar"
        done

        if [[ "$_got" != "1" ]]; then
            echo -e "${Y}⚠${W}  Không tải được LLVM ${_fv} — thử phiên bản khác"
            continue
        fi

        echo -e "${B}ℹ${W}  Giải nén..."
        local _extract_tmp; _extract_tmp="$(mktemp -d)"
        if ! tar xf "$_tar" -C "$_extract_tmp" 2>/tmp/bolt-llvm-extract.log; then
            echo -e "${R}✘${W}  Giải nén thất bại"
            rm -rf "$_extract_tmp"; rm -f "$_tar"
            continue
        fi

        local _top_dir
        _top_dir="$(find "$_extract_tmp" -mindepth 1 -maxdepth 1 -type d | head -1)"
        if [[ -z "$_top_dir" || ! -x "$_top_dir/bin/llvm-bolt" ]]; then
            echo -e "${Y}⚠${W}  Gói LLVM ${_fv} không có llvm-bolt trong bin/ — bỏ qua"
            rm -rf "$_extract_tmp"; rm -f "$_tar"
            continue
        fi

        mkdir -p "$_install_dir"
        cp -a "$_top_dir/bin" "$_install_dir/bin"
        [[ -d "$_top_dir/lib" ]] && cp -a "$_top_dir/lib" "$_install_dir/lib"
        rm -rf "$_extract_tmp"
        rm -f "$_tar"   # xoá tarball lớn sau khi giải nén — chỉ giữ bin/+lib/

        if [[ ! -x "$_install_dir/bin/llvm-bolt" ]]; then
            echo -e "${R}✘${W}  llvm-bolt không thực thi được sau giải nén"
            continue
        fi

        echo -e "${G}✔${W}  LLVM ${_fv} (BOLT) sẵn sàng: ${_install_dir}"
        _bolt_link_prebuilt "$_v" "$_install_dir" && return 0
    done

    echo -e "${R}✘${W}  Không tải được bản LLVM nào có BOLT — kiểm tra kết nối mạng"
    return 1
}
...
_bolt_check_tools() {
    ...
    if [[ -z "$(_bolt_find_tool llvm-bolt)" || -z "$(_bolt_find_tool merge-fdata)" ]]; then
        # apt không có (hoặc thiếu) llvm-bolt/merge-fdata — thử tải bản
        # LLVM prebuilt chính thức từ GitHub Releases thay vì bỏ cuộc
        echo -e "${Y}⚠${W}  apt không có llvm-bolt/merge-fdata — thử tải LLVM prebuilt (ổn định hơn)..."
        _bolt_download_prebuilt_llvm || return 1
    fi
...
_bolt_ensure_runtime_lib() {
    ...
    _found=$(find /usr/lib /usr/lib64 /usr/local/lib \
                 ...
                 "${BOLT_PREBUILT_ROOT:-/nonexistent}"/*/lib \
                 -maxdepth 5 -name "$_libname" -type f 2>/dev/null | head -1) || true

...
_resolve_qemu_bin() {
    local _real_home=""
    [[ -n "${SUDO_USER:-}" ]] && _real_home=$(getent passwd "$SUDO_USER" 2>/dev/null | cut -d: -f6)
    for q in \
        "${QEMU_BIN:-}" \
        "$HOME/qemu-static/bin/qemu-system-x86_64" \
        "$HOME/qemu-optimized/bin/qemu-system-x86_64" \
        "${_real_home:-}/qemu-optimized/bin/qemu-system-x86_64" \
        "/home/jovyan/qemu-optimized/bin/qemu-system-x86_64" \
        "/opt/qemu-optimized/bin/qemu-system-x86_64" \
        "$(command -v qemu-system-x86_64 2>/dev/null)"; do
        # -x chỉ kiểm tra permission bit — KHÔNG phát hiện được mount noexec.
        # Phải thử --version thật để xác nhận binary chạy được.
        [[ -n "$q" && -x "$q" ]] && "$q" --version &>/dev/null && { echo "$q"; return 0; }
    done
    return 1
}
...
_resolve_qemu_img() {
    for qi in \
        ...
        "/home/jovyan/qemu-optimized/bin/qemu-img" \
...
# ════════════════════════════════════════════════════════════════
#  EXEC-MOUNT PROBE
#  Một số container HPC/JupyterHub mount noexec trên toàn bộ root fs
#  (/opt, /root, /usr...) và chỉ cho phép thực thi trên volume home
#  bền vững (vd /home/jovyan). Permission bit +x KHÔNG đủ để biết —
#  phải thử thực thi thật để phát hiện noexec.
# ════════════════════════════════════════════════════════════════
_dir_allows_exec() {
    local d="$1"
    [[ -n "$d" ]] || return 1
...
# In ra thư mục đầu tiên vừa ghi được vừa thực thi được (dò qua noexec)
_find_exec_install_dir() {
    local real_home=""
    if [[ -n "${SUDO_USER:-}" ]]; then
...
_qemu_build_tuning() {
    ...
    if _bolt_check_tools 2>/dev/null; then
        _bolt_active=1
        QEMU_BASE_LDFLAGS="-Wl,-O1 -Wl,--as-needed -Wl,--emit-relocs -Wl,-z,noexecstack"
    else
        QEMU_BASE_LDFLAGS="-Wl,-O1 -Wl,--as-needed -Wl,--gc-sections -Wl,-z,noexecstack"
    fi
...
if [[ "$choice" == "y" ]]; then
    ...
    if [[ "$ROOTLESS" == "1" ]]; then
        ...
        _rootless_build
         ...
         && "/opt/qemu-optimized/bin/qemu-system-x86_64" --version &>/dev/null; then
        ...
        export LD_LIBRARY_PATH="/opt/qemu-optimized/lib:${LD_LIBRARY_PATH:-}"
         ...
         && "$HOME/qemu-optimized/bin/qemu-system-x86_64" --version &>/dev/null; then
    ...
    elif [[ -x "/home/jovyan/qemu-optimized/bin/qemu-system-x86_64" && "$AUTO_BUILD" != "yes" ]] \
         && "/home/jovyan/qemu-optimized/bin/qemu-system-x86_64" --version &>/dev/null; then
        BUILT_VER=$("/home/jovyan/qemu-optimized/bin/qemu-system-x86_64" --version 2>/dev/null \
            | head -1 | grep -oE '[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown")
        echo -e "${G}⚡ QEMU v${BUILT_VER} đã có tại /home/jovyan/qemu-optimized — bỏ qua build${W}"
        export QEMU_BIN="/home/jovyan/qemu-optimized/bin/qemu-system-x86_64"
        export PATH="/home/jovyan/qemu-optimized/bin:$PATH"
    elif [[ -x "$QEMU_BIN" && "$AUTO_BUILD" != "yes" ]] && "$QEMU_BIN" --version &>/dev/null; then
...
" 2>/dev/null
        ...
        :
        {
            ...
            echo -e "${R}✘${W} pip_install (meson/ninja) thất bại — xem log:"
            tail -30 /tmp/pip-install.log
            exit 1
        }
...
"
        ...
        echo -e "${B}ℹ${W}  Cài đặt QEMU..."
        _install_dir=$(_find_exec_install_dir) || {
            echo -e "${R}✘${W} Không tìm được thư mục nào vừa ghi được vừa thực thi được (mọi mount đều noexec?) — không thể cài QEMU."
            exit 1
        }
        _qemu_install_prefix="$_install_dir/qemu-optimized"
        echo -e "${B}ℹ${W}  → cài vào ${_qemu_install_prefix} (đã kiểm tra thực thi được)"
        mkdir -p "$_qemu_install_prefix"

        if [[ $EUID -eq 0 ]] || sudo -n true 2>/dev/null; then
            _sudo_pfx=""
            [[ $EUID -ne 0 ]] && _sudo_pfx="sudo"
            $_sudo_pfx env MESON_INSTALL_DESTDIR_PREFIX="$_qemu_install_prefix" ninja install \
                > /tmp/qemu-install.log 2>&1 \
                || DESTDIR="" MESON_INSTALL_DESTDIR_PREFIX="$_qemu_install_prefix" ninja install \
                    >> /tmp/qemu-install.log 2>&1 \
                || { echo -e "${R}✘${W} ninja install thất bại:"; tail -20 /tmp/qemu-install.log; exit 1; }
        else
            MESON_INSTALL_DESTDIR_PREFIX="$_qemu_install_prefix" ninja install \
                > /tmp/qemu-install.log 2>&1 \
                || { echo -e "${R}✘${W} ninja install thất bại:"; tail -20 /tmp/qemu-install.log; exit 1; }
        fi

        # Xác nhận binary thực sự chạy được sau khi cài (không chỉ tin +x bit)
        if ! "$_qemu_install_prefix/bin/qemu-system-x86_64" --version &>/dev/null; then
            echo -e "${R}✘${W} Cài xong nhưng binary vẫn không thực thi được tại ${_qemu_install_prefix} — kiểm tra lại mount/permission thủ công."
            exit 1
        fi
        export PATH="$_qemu_install_prefix/bin:$PATH"
        export QEMU_BIN="$_qemu_install_prefix/bin/qemu-system-x86_64"
        echo -e "${G}✔${W} Cài đặt QEMU xong → ${_qemu_install_prefix}"

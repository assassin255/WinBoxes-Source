...
_resolve_qemu_bin() {
    ...
    for q in \
        ...
        [[ -z "$q" || ! -x "$q" ]] && continue
        # -x chỉ kiểm tra permission bit, KHÔNG phát hiện được mount
        # noexec (thường gặp ở /opt trong container JupyterHub/HPC).
        # Phải verify bằng cách chạy thật --version mới chắc chắn exec được.
    ...
    # Nếu tới đây: có binary với x-bit nhưng không exec được → rất có thể
    # do noexec mount (ví dụ /opt). Thử tự copy sang $HOME (thường không
    # bị noexec) rồi dùng bản đó thay vì báo lỗi luôn.
    local _src_dir
    for _src_dir in "/opt/qemu-optimized" "$HOME/qemu-optimized"; do
        local _src_bin="$_src_dir/bin/qemu-system-x86_64"
        [[ -x "$_src_bin" ]] || continue
        local _dst_dir="$HOME/.qemu-optimized-noexec-fallback"
        local _dst_bin="$_dst_dir/bin/qemu-system-x86_64"
        if [[ ! -x "$_dst_bin" ]]; then
            echo -e "${Y}⚠${W}  $_src_dir có vẻ nằm trên mount noexec — copy sang $_dst_dir..." >&2
            mkdir -p "$_dst_dir" 2>/dev/null
            cp -a "$_src_dir/." "$_dst_dir/" 2>/dev/null
            chmod -R u+rx "$_dst_dir/bin" "$_dst_dir/lib" 2>/dev/null
        fi
        if [[ -x "$_dst_bin" ]] && "$_dst_bin" --version >/dev/null 2>&1; then
            echo "$_dst_bin"
            return 0
        fi
    done
...
# Luôn thêm lib-dir của chính QEMU_BIN đã resolve (không chỉ riêng
# case qemu-static) — quan trọng khi _resolve_qemu_bin() vừa fallback
# copy binary sang $HOME/.qemu-optimized-noexec-fallback do /opt bị
# mount noexec: prefix cũ (/opt/qemu-optimized/lib) không còn đúng nữa.
